...
export class ProductFormComponent implements OnInit, OnDestroy {
        ...
        .subscribe({
          next: (response) => {
            if (response) {
              this.messageService.add({
                severity: 'success',
                summary: 'Success',
                detail: `Product ${response.name} criado com sucesso!`,
                life: 2500,
              });
            }
          },
          error: (err) => {
            ...
            this.messageService.add({
              severity: 'error',
              summary: 'Erro',
              detail: 'Error ao criar o produto!',
            });
          },
        });
    }
        ...
        .subscribe({
          next: () => {
            this.messageService.add({
              severity: 'success',
              summary: 'Success',
              detail: `Product editado com sucesso!`,
              life: 2500,
            });
            this.editProductForm.reset();
          },
          error: (err) => {
            console.error('Erro na edição do produto:', err);
            this.messageService.add({
              severity: 'error',
              summary: 'Erro',
  ...
  getProductSelectedDatas(productId: string): void {
    ...
    if (allProducts.length > 0) {
      ...
      if (productFiltered) {
        this.saleProductSelected = productFiltered[0];

        this.editProductForm.setValue({
          name: this.saleProductSelected?.name,
          price: this.saleProductSelected?.price,
          amount: this.saleProductSelected?.amount,
          description: this.saleProductSelected?.description,
          category_id: this.saleProductSelected?.category.id,
        });
      }
    }
  }
      ...
      .subscribe({
        next: (response) => {
          if (response.length > 0) {
            ...
            this.productsDatas &&
              this.productDtService.setProductsData(this.productsDatas);
          }
        },
      });
  }
